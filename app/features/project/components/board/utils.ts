import type { Task, TaskPriority, TaskStatus } from '~/core/task/task.types';
...
function sortByOrder(items: Task[], order: ProjectViewState['order']) {
  if (order === 'manual') {
...
export function buildBoardState(tasks: Task[], order: ProjectViewState['order']): BoardState {
  return {
...
export function buildBoardSourceKey(tasks: Task[], order: ProjectViewState['order']) {
  return `${order}:${tasks
